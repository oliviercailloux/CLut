...
In the collaborative learning community, this is usually taken to mean: recommend an item among the most preferred for the user \commentYM{the most preferred one(s) ? qu'entendez-vous ici par "preferred" ? Je pense que, si vous voulez vraiment utiliser le terme ici, il faut lui donner une définition technique pour que le lecteur comprenne bien ce vis-à-vis de quoi vous vous démarquer}. In this article, we want to propose another goal for recommender systems. We keep the global aim of recommending some item as most appropriate for a given user, but propose a different understanding of appropriateness. The goal we propose is to recommend an item among those that are in the \ac{DP} \commentYM{je vois que vous reprenez D PREFERENCE au lieu de JUDGMENT, ce qui renforce la difficulté de comprendre ce que signifie de "preferred" que vous rejetez}of the user, and to prove that it does \commentYM{that what does what?}. The \ac{DP} of the user captures what she prefers when considering all arguments in favor or against all possible items. This differs from her intuitive preference \commentYM{donc le "preferred" de tout à l'heure est "intuitif"? c'est ce qu'on appelait "shallow" dans une version du papier DJ?} in that the latter does not consider arguments. Thus, the goal we propose is appropriate when one is interested to help the user form a deliberated judgment about which item is best, rather than predict \commentYM{identify or guess or predict?
je pense que predict est plus dangereux} which item the user intuitively prefers \commentYM{peut-être expliquer pourquoi on peut avoir envie de se donner un tel but ?!}. Because this will be done using systems that argue, we call the kind of system we define here an \ac{AR} \commentYM{argumentative recommender ? pb d'acronymes}.
Further motivations for this new goal are described in \cref{sec:motiv}, and it is precisely defined in \cref{sec:def}.

Together with this new goal, it is necessary to propose a corresponding measure of the performance of the recommender system (done in \cref{sec:pred}). In the classical approach, measuring the performance of the system amounts to compare its claims (the items it proposes as preferred) to reality (the preference of the user). Because the \ac{DP} of the user is not directly observable, this direct check of correspondance becomes non applicable. However, as will be showed \commentYM{shown}, the problem of evaluating the quality of an \ac{AR} can be framed as a prediction problem (although involving different objects), thereby recovering the possibility of empirical validation by confrontation between claims of the system and reality.
...
\section{Motivation}\commentYM{je trouve ça un peu dérangeant d'avoir une section motivation. Pour moi la motivation a sa place en introduction. }
\label{sec:motiv}
This article discusses a notion of preference that possibly changes when confronted to arguments. That is, we assume that the user’s knowledge of which item is best for him is not necessarily fixed a priori, and in particular, that the user may change his mind \commentYM{Comme il y a une littérateurs sur la performativité des indicateurs, il doit bien y avoir une littérature sur la performativité des systèmes de recommandation, non ?} when presented with arguments in favor of, or counter-arguments against, various items. \commentYM{qu'est-ce qu'un argument en faveur d'un item ? D'ailleurs qu'est-ce qu'un item ? C'est un objet à acheter ? Et un argument en faveur d'un item est un argument pour acheter cet objet ? Si oui, pourquoi ne pas le dire comme ça ? Si c'est + subtile, il faut peut-être expliquer la subtilité}

By contrast, the notion of preference considered by classical recommender systems (usually left implicit) is that the user intuitively knows what he prefers, and that this is the right basis for him to decide \commentYM{je dirais plutôt que ce qui est postulé, c'est que c'est la bonne base pour lui recommander quelque chose, parce qu'on veut, c'est qu'il achète, peu importe que ce soit la bonne base pour lui pour décider}. This notion can be related to the conception of preference put forth \commentYM{forward?} by \citet[p. 16]{von_neumann_theory_1944} in their seminal work (cited by \citet{fishburn_retrospective_1989}). They write that “It is clear that every measurement – or rather every claim of measurability – must ultimately be based on some immediate sensation (…). In the case of utility the immediate sensation of preference (…) provides this basis.” \commentYM{je ne comprends pas très bien pourquoi ce choix de citation ? Parce que le terme "sensation" semble se rapprocher de l'idée de préférence "intuitive", et donc d'une certaine "immédiateté" qui ne laisserait pas la place à une possible évolution des préférences ?
selon moi ce sont des débats différents. Une sensation peut être immédiate, et pourtant évolutive, voire construite et réactive vis-à-vis d'arguments.
Par ailleurs l'idée que la notion de préférence fondant les systèmes de recommandation classiques est une notion de préférence rattachée qui peut faire l'objet d'une "sensation de préférence" me parait discutable. En tout cas il me semble qu'on ne peut pas l'affirmer sans au moins évoquer la célébrissime discussion de Meinard \& Gharbi 2018 entre compréhension substantielle et analytique de la notion de préférence !} When applied to collaborative filtering, this view suggests that the user does not need help for comparing pairs of item: the job of the system is merely to relieve the user from having to search through the whole set. 
The user, when presented with a pair of items, knows which one he prefers by introspection \commentYM{en parlant d'introspection, vous vous éloignez de l'idée de sensation immédiate, par une autre route....}, possibly after having tried them both. %DEL?
Similarly, when the user has already provided some comparisons of preference, this is the final word about those pairs.

This notion of preference as “intuitive” preference is not always the best basis to ground recommendations \commentYM{j'ajouterais "provided that the aim of recommending is to point out what is best for the individual concerned"}. Indeed, the user might be unwilling to judge what is best for her on the sole basis of her unaided intuition. First, there are cases where the user can’t easily try out the items. An example is a non repeatable choice, such as choosing a university to study in. 
Second, the best choice might be the result of a complex thought process that should better ensure, as much as possible, that all relevant arguments have been considered, rather than purely rely on intuition \commentYM{Je ne comprends rien à cette phrase. Proposition : Arguably, at least in some cases, identifying the best choice requires deploying a complex thought process designed to ensure that all the relevant argument have been taken into account}.
Consider the decision of which smartphone, or which house, to buy. 
This is also easily \commentYM{?} the case when a notion of fairness is involved, for example when determining who will receive a prize, or how to best distribute \commentYM{how to distribute?}revenue in a society, or to which cause I should donate money.
Third, the user might appreciate being pointed to some non-salient feature of the items under comparison; for example, a user might feel an intuitive attraction to the plane alternative when chosing a transportation means for holidays, seeing that the flight time is only one hour compared to three hours by train, but change her mind when being reminded that the total travel time should be taken into account. Finally, an argument coming from psychology is that unaided intuition is known to be sensible, in some contexts, to framing effects that would perhaps not be considered relevant (by the user himself) when considering arguments and counter-arguments \citep{kahneman_thinking_2013}. An example that involves several kinds of the just enumerated concerns \commentYM{of the above mentioned concerns} is obtaining a decision procedure for credit requests in a bank \commentYM{is how to devise a decision procedure to adjudicate credit requests}: the user \commentYM{who are you talking about? the creditor I guess?} probably wants to involve \commentYM{include?} fairness considerations in order to avoid (possibly unconscious) appearance \commentYM{?} of unjust discriminations; and even if only profit considerations are to bear, the user might want the procedure to go beyond reflecting the bare intuition of an expert \commentYM{phrase très dure à suivre}.

\section{Definition of deliberated preference}
\label{sec:def}
This section describes the new understanding of preference I \commentYM{vous ne faites plus qu'un ?} propose, called the \ac{DP} of the user (it is a simplification of the recently proposed concept of deliberated judgment \citep{cailloux_formal_2018}).

I consider given a set of items $\allitems$ among which a user $\usr$ wants to choose. I also consider given a set $\allargs$ containing arguments that may possibly help $\usr$ form a deliberated opinion about which item is best for her. Elements of $\allargs$ are not detailed and can be conceived as strings in a natural language. An example of an argument is $\ar = $ “Item $\itm$ is better than item $\itm'$ because $\itm$ has a good performance on criteria ‘price’ and ‘speed’ while item $\itm'$ has a good performance only on criterion ‘aspect’, which you \commentYM{"I" ? l'argument est quelque chose que "i" doit s'approprier, non ?}do not consider important”.
...
Define $(\itm, \ar) \ibeats (\itm', \ar')$ iff $\usr$ strictly prefers $\itm$ to $\itm'$, given $\ar$ and $\ar'$. Note that if no strict preference holds, $\ibeats$ does not relate these two pairs (thus $(\itm, \ar) \nibeats (\itm', \ar')$ and $(\itm', \ar') \nibeats (\itm, \ar)$). If the answer of the individual changes when asked several times the same question, we consider that no strict preference holds, as we are interested in the stable part of the individual’s judgement. \commentYM{preference?}
The relation $\ibeats$ plays here the role of the basic observable \commentYM{the basic observable preference ?} \citeauthor{von_neumann_theory_1944} talk about (see \cref{sec:motiv}).
...
Formally, an \ac{AR} $\eta$ has a \emph{scope} $\allusers$, representing a set of users it claims to be able to predict the \ac{DP} of \commentYM{a set of users whose DP it claims to be able to predict?}, and gives, for a \commentYM{any?} user $\usr \in \allusers$, a tuple $(\mitems, \mdef, R_\eta, \matt)$, where $\mitems \subseteq \allitems$ is a set of items that $\eta$ claims are among the \ac{DP} of $\usr$, $\mdef: \mitems × \allitems → \allargs$ is an argumentation strategy \commentYM{est-ce que cette terminologie fait écho à un usage courant en théorie de l'argumentation? Si oui, il faudrait une ref, sinon ça mériterait quelques mots, parce que je pense que dans le langage courant ``argumentation strategy'' fait référence à des chose comme des séquences temporelles d'utilisations d'arguments, des attentes relatives aux arguments que les interlocuteurs vont évoquer, etc.} used to defend items in $\mitems$, $R_\eta \subseteq \allitems × \allitems$ is a binary relation that contains pairs of items $(\itm, \itm')$ such that $\eta$ claims that $\usr$ deliberately prefers $\itm$ to $\itm'$, and $\matt: R_\eta → \allargs$ is an argumentation strategy used to support the claims represented by $R_\eta$. (Elements of this tuple depend on $i$, though this is omitted from the notation.) \commentYM{je pense que ce paragraphe mériterait quelques explications supplémentaires, car moi j'ai eu besoin de lire le suivant pour comprendre pourquoi tu avais besoin à la fois de $\mdef$ et de $\matt$}

An \ac{AR} is valid if its functions $\mdef$ and $\matt$ indeed justify its claims. Informally, it is required of $\mdef$ that, when given an item $\itm'$ and given an item $\itm \in \mitems$ considered by $\eta$ as being \commentYM{to be} in the \ac{DP} of $i$, $\mdef$ produces an argument that successfully defends $\itm$, whatever the argument given in favor of $\itm'$. More precisely and completely, an \ac{AR} $\eta$ is said to be \emph{correct} about a user $\usr$ iff $\forall \itm \in \mitems, \itm' \in \allitems, \ar' \in \allargs: (\itm', \ar') \nibeats (\itm, \mdef(\itm, \itm'))$ and $\forall (\itm, \itm') \in R_\eta, \ar' \in \allargs: (\itm, \matt(\itm, \itm')) \ibeats (\itm', \ar')$. An \ac{AR} is \emph{valid} iff it is correct about all users in its scope. \commentYM{je ne vois pas l'intérêt de distinguer valid de correct, au lieu de dire valide pour $i$ vs valid pour $\allusers$}

Importantly, validity implies the correctness of the claims concerning the \ac{DP} of $\usr$ \commentYM{je ne comprends pas ce que tu dis ici. Jusqu'ici il était question de modèle correct, et maintenant tu parles de correctness de ses ``claims''. Et tu définis valide par ``correct pour le monde'', donc je ne comprends pas comment dire ``si valide alors correct'' peut être autre chose que trivial} This fact is stated formally below, with $\mnitems = R_\eta(\allitems) = \set{\itm' \in \allitems \suchthat (\itm, \itm') \in R_\eta \text{ for some } \itm \in \allitems}$. (The proof is omitted.) \commentYM{quel intérêt d'introduire la notation $\mnitems$ si elle veut juste dire $R_\eta(\allitems)$?}
\begin{theorem}
...
Yet other approaches propose to use argumentation theory to enhance recommender systems \citep{chesnevar_empowering_2009, rago_argumentation-based_2018}. These trends constitute promising areas of investigation for building \acp{AR}. Such articles typically are focused towards producing \commentYM{focused on producing/concerned with producing?} recommender systems on the basis of some principles chosen by the authors. By comparison, this article proposes a precise definition of a new goal for similar argumentative systems (which may or may not be similar to the goal the authors of the cited articles have in mind) \commentYM{là tu dis juste que les autres ne définissent pas clairement leurs principes? vraiment ?}, and a way of comparing \acp{AR} experimentally when their predictions disagree.
...
\citet{johnson_logistic_2014} uses logits for matrix factorization. \commentOC{Should we say one word about this?}
...
\label{sec:princ}
\Cref{sec:def} described a new kind of recommender system, that consider the recommendation task under a different light, and \cref{sec:pred} described how to evaluate such \acp{AR}. But such \acp{AR} do not exist yet. One way of building such systems consists in adapting existing works (related references are given in \cref{sec:litt}). Another way is to design an \ac{AR} from the ground up specifically for tackling the goal described here \commentYM{I'd say ``tackling the task''/''reaching a goal''}. Although it is not the main point of this conceptual article, roughly sketching a possible way of doing this could help make this proposal more concrete. This section first briefly describes such a possible way in the context of collaborative settings, then indicates some conditions on the context that must be fulfilled in order to make the proposed approach applicable. \commentYM{si c'est ça, le nom de la section n'est guère explicite}The next section more concretely sketches a way of learning such a model and of studying its performances with numerical experiments.
...
The weak order ${\succeq}$ corresponds to $R_\eta$ and its maximal elements correspond to $\mitems$ (though the definition tolerates other possibilities). Decision theory has proposed models to represent preferences of decision makers. Even though decision theory does not explicitly use the concept of deliberated preferences as defined here, because its models are conceived for grounding decisions in sound principles, they might be adequate to model deliberated preferences. \commentYM{ce que vous dites ici me semble extrêmement ambigu, ou alors gratuit, ou alors ça ouvre une boite de Pandore. Je dirais qu'il y a un enjeu rhétorique important ici. Il faut qu'on comprenne pourquoi ce paragraphe ne démonte pas complètement toute l'originalité et tout l'intérêt de la démarche}
...
This is now a classical learning task that can be achieved by classical means. \commentOC{Florian: perhaps say (in one sentence?) what we do here, with a reference if appropriate?}
...
For illustrative purposes, we describe here the Sushi dataset \citep{kamishima_nantonac_2003}. It is not fully appropriate for our task because it has very few objective descriptors, and because the task of appreciating sushis probably does not mandate much deliberation. But it is formally applicable in our setting as it contains several users evaluating a set of items whose objective description is (unfortunately only partially) known. \commentOCf{Je crois que la description n’apporte rien. On ferait peut-être mieux de citer simplement ce jeu de données, et de s’arrêter là, en attendant d’en faire vraiment quelque chose. Si on la garde, il faut décider si on supprime les descriptions des données qu’on n’utilise pas, tel que $>^\text{top}_i$. En fait, plus j’y pense, et plus je trouve qu’on ferait mieux de le laisser tomber complètement, car il n’est vraiment pas approprié pour nous (on ne sait même pas si la description “frequency”, par exemple, représente vraiment qqch. d’objectif et d’où elle vient). Je propose donc de supprimer ce § et ce qui suit.}
