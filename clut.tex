...
\section{Motivation}
\label{sec:motiv}
\commentYM{je trouve ça un peu dérangeant d'avoir une section motivation. Pour moi la motivation a sa place en introduction. }
This article discusses a notion of preference that possibly changes when confronted to arguments. That is, we assume that the user’s knowledge of which item is best for him is not necessarily fixed a priori, and in particular, that the user may change his mind \commentYM{Comme il y a une littérateurs sur la performativité des indicateurs, il doit bien y avoir une littérature sur la performativité des systèmes de recommandation, non ?} when presented with arguments in favor of, or counter-arguments against, adopting various items. \commentYM{qu'est-ce qu'un argument en faveur d'un item ? D'ailleurs qu'est-ce qu'un item ? C'est un objet à acheter ? Et un argument en faveur d'un item est un argument pour acheter cet objet ? Si oui, pourquoi ne pas le dire comme ça ? Si c'est + subtile, il faut peut-être expliquer la subtilité}\commentOCf{On n’argumente pas en faveur d’un item mais d’un acte.}

By contrast, the notion of preference considered by classical recommender systems (usually left implicit) is that the user intuitively knows what he prefers, and that this is the right basis for him to decide. This notion can be related to the conception of preference put forth by \citet[p. 16]{von_neumann_theory_1944} in their seminal work (cited by \citet{fishburn_retrospective_1989}). They write that “It is clear that every measurement – or rather every claim of measurability – must ultimately be based on some immediate sensation (…). In the case of utility the immediate sensation of preference (…) provides this basis.” \commentYM{je ne comprends pas très bien pourquoi ce choix de citation ? Parce que le terme "sensation" semble se rapprocher de l'idée de préférence "intuitive", et donc d'une certaine "immédiateté" qui ne laisserait pas la place à une possible évolution des préférences ?
...
The user, when presented with a pair of items, knows immediately which one he prefers \commentYM{en parlant d'introspection, vous vous éloignez de l'idée de sensation immédiate, par une autre route....}, possibly after having tried them both.
...
Importantly, correctness about $i$’s attitude towards arguments implies the validity of the claims concerning the \ac{DP} of $\usr$. This fact is stated formally below, where $R_\eta(\allitems) = \set{\itm' \in \allitems \suchthat (\itm, \itm') \in R_\eta \text{ for some } \itm \in \allitems}$ represents the items postulated by $\eta$ as being less good than some other item for $i$. (The proof is omitted.) \commentYM{quel intérêt d'introduire la notation $\mnitems$ si elle veut juste dire $R_\eta(\allitems)$?}
\begin{theorem}
...
It is important to realize that the general part of this article (\cref{sec:motiv,sec:def,sec:pred}) is not conceptually dependent on its most concrete parts (\cref{sec:princ,sec:tech}). \Acp{AR} could be built on different bases than decision-theoretic models. For example, gathering existing arguments in textual forms on various sources on the web and displaying them as-is to the user, or using natural language processing techniques. Similarly, the concept could be applied to settings different than collaborative learning, or using learning techniques different than the one \cref{sec:tech} investigates.
