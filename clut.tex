...
In the double-strategy, we use different dimensions: $k = k^W1 + k^u = k^{W2} + k^v$.
The matrix $W_A$ has size $(k^{W1}, d_A)$ and $U$ has size $(m, k^u)$. $W_B$ has size $(k^{W2}, d_B)$ and $V$ has size $(k^v, n)$. $r_{ij} = \mu_i^T \nu_j$ where $\mu_i$ is the $k$-dimensional vector composed of $W_A a_i$ then $u_i$ and $\nu_j$ is composed of $W_B b_i$ then $v_j$.

We should compare this approach to the more classical one.
(Theoretical question to study: difference between this and the above approach?)

\section{Reducing parameters}
The matrix $W_B$ could be constrained to having mostly zeroes, to make sure that indeed the attributes of the items are used in their $\mu$-vectorial description. We could even take $\mu = d_B$ and make $W_B$ a diagonal matrix. (Is there any benefit not to do so?)

This suggests the following simple approach: concatenate $b_j$ to $v_j$ to obtain $\nu_j$. Don’t use $W_B$ (or set it to identity). Use $W_A a_i$ of size $k = d_B + k^v$; obtain also $u_i$ of size $k$, add both with a big weight on the first one, and on the first $d_B$ components, if we want to explain.
