...
Theorem (to be checked): if the preference of the user $\succeq$ satisfies all the above conditions, then there exists a value function $v: \allitems → \R$ and partial value functions $v_c: X_c → \R$ for each criterion $c$ such that for any two items, $j \succeq j' ⇔ v(j) ≥ v(j')$ and $v(j) = \sum_{c \in \crits} v_c(b_c(j))$.%, and the partial value functions $v_c$ are all increasing in $\succ_c$: if $x_c \succ_c x_{c'}$ then $v_c(x_c) \succ_c v_c(x_{c'})$.
